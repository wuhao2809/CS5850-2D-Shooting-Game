...
namespace game {
    ...
    GameEngine::GameEngine(const std::string& title, const std::string& assetsDir)
        ...
        : window(nullptr), renderer(nullptr), font(nullptr),
          ...
          width(800), height(600), title(title), assetsDirectory(assetsDir), running(false), 
          timer(60), hud(std::make_unique<HUD>(width, height, &timer)), gameWorld(nullptr) {}
#else
    ...
    bool GameEngine::init() {
        ...
        // Configure logging levels - default to WARN (shows nothing), ESC cycles to INFO for debugging
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Configuring logging system...");
        SDL_SetLogPriorities(SDL_LOG_PRIORITY_WARN);  // Set default to WARN (silent)
        
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Logging configured: Default=WARN (silent), ESC toggles to INFO for debugging");
...
#ifdef __APPLE__
        font = TTF_OpenFont("/System/Library/Fonts/Helvetica.ttc", 24);  // Use Helvetica on macOS
#else
        font = TTF_OpenFont("C:\\Windows\\Fonts\\arial.ttf", 24);  // Use Arial on Windows
#endif
        if (!font) {
            SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Font Loading Error: %s", SDL_GetError());
            TTF_Quit();
            SDL_Quit();
            return false;
        }
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "[GameEngine] Setting renderer in GameWorld: %p", renderer);
        
        gameWorld->setAssetsDirectory(assetsDirectory);  // Set assets directory before initialization
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "[GameEngine] World dimensions from GameWorld: %dx%d", width, height);
        ...
        if (!window) {
            SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Window Creation Error: %s", SDL_GetError());
            SDL_Quit();
            return false;
        }
        ...
        if (!renderer) {
            SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Renderer Creation Error: %s", SDL_GetError());
        ...
        // Set up HUD elements
        if (hud) {
            // Professional GameHUD handles all display elements automatically
            // No manual text element setup needed
        }
    ...
    void GameEngine::handleEvents() {
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_EVENT_QUIT) {
                ...
                SDL_LogInfo(SDL_LOG_CATEGORY_INPUT, "[GameEngine] Key pressed: %s (SDL key code: %d)", keyNameLower.c_str(), event.key.key);
                ...
                SDL_LogInfo(SDL_LOG_CATEGORY_INPUT, "[GameEngine] Publishing keyboard event - Key: %s, Pressed: true", keyNameLower.c_str());
                events::EventManager::getInstance().publish(keyboardEvent);

                if (event.key.key == SDLK_Q) {
                    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "[GameEngine] Quit key (Q) pressed, stopping game loop");
                    running = false;
                }
                else if (event.key.key == SDLK_H) {
                    if (hud) {
                        hud->toggleVisibility();
                        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "[GameEngine] HUD visibility toggled");
                    }
                }
                    ...
                    static int currentLogLevelIndex = 2;  // Start with WARN (our default)
                    ...
                    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                        ...
                        newLevel == SDL_LOG_PRIORITY_VERBOSE ? "VERBOSE" : "UNKNOWN");
                    
                    // Professional GameHUD handles all display elements automatically
                    // No manual text updates needed
                    
                    // Move to next level
                    currentLogLevelIndex = (currentLogLevelIndex + 1) % (sizeof(logLevels) / sizeof(logLevels[0]));
                }
            }
                ...
                std::string keyNameLower = std::string(keyName);
                std::transform(keyNameLower.begin(), keyNameLower.end(), keyNameLower.begin(), ::tolower);
                SDL_LogInfo(SDL_LOG_CATEGORY_INPUT, "[GameEngine] Key released: %s (SDL key code: %d)", keyNameLower.c_str(), event.key.key);
                ...
                SDL_LogInfo(SDL_LOG_CATEGORY_INPUT, "[GameEngine] Publishing keyboard event - Key: %s, Pressed: false", keyNameLower.c_str());
                events::EventManager::getInstance().publish(keyboardEvent);
            }
        }
    ...
    void GameEngine::update() {
        ...
        // Update the HUD with deltaTime
        if (hud) {
            hud->update(timer.getAverageFrameTime());
        }
    }
    ...
    void GameEngine::display() {
        SDL_LogInfo(SDL_LOG_CATEGORY_RENDER, "Starting display phase...");
        
        // Render HUD if it exists
        if (hud) {
            SDL_LogInfo(SDL_LOG_CATEGORY_RENDER, "Rendering HUD...");
            hud->render(renderer);
            
            // Render collision debug if debug overlay is enabled
            if (hud->getDebugOverlay()) {
                auto collisionComponents = gameWorld->getCollisionComponents();
                hud->renderCollisionDebug(renderer, collisionComponents);
            }
        }
        
        // Present the frame to the screen
        SDL_LogInfo(SDL_LOG_CATEGORY_RENDER, "Presenting frame...");
        present();
    ...
    void GameEngine::renderText(const std::string& text, int x, int y) {
        ...
        textSurface = TTF_RenderText_Solid(font, text.c_str(), text.length(), textColor);
        if (!textSurface) {
            SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Surface Creation Error: %s", SDL_GetError());
            return;
        }
