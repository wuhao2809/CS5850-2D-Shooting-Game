...
namespace game {
    ...
    GameEngine::GameEngine(const std::string& title, const std::string& assetsDir)
        ...
        : window(nullptr), renderer(nullptr), font(nullptr),
          ...
          timer(60), hud(std::make_unique<HUD>(width, height)), gameWorld(nullptr) {}
#else
    ...
    bool GameEngine::init() {
        ...
        if (hud) {
            ...
            // Add welcome message
            const std::string welcomeText = "Welcome to the Game!";
            const int welcomeFontSize = 24;
            // Calculate center position
            const int welcomeX = (width - welcomeText.length() * welcomeFontSize / 2) / 2;
            const int welcomeY = (height - welcomeFontSize) / 2;
            hud->addText("welcome", welcomeText, welcomeX, welcomeY, welcomeFontSize);
    ...
    void GameEngine::handleEvents() {
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_EVENT_QUIT) {
                ...
                SDL_LogVerbose(SDL_LOG_CATEGORY_INPUT, "[GameEngine] Key pressed: %s (SDL key code: %d)", keyNameLower.c_str(), event.key.key);
                ...
                SDL_LogWarn(SDL_LOG_CATEGORY_INPUT, "[GameEngine] Publishing keyboard event - Key: %s, Pressed: true", keyNameLower.c_str());
                ...
                std::string keyNameLower = std::string(keyName);
                std::transform(keyNameLower.begin(), keyNameLower.end(), keyNameLower.begin(), ::tolower);
                SDL_LogVerbose(SDL_LOG_CATEGORY_INPUT, "[GameEngine] Key released: %s (SDL key code: %d)", keyNameLower.c_str(), event.key.key);
                ...
                SDL_LogWarn(SDL_LOG_CATEGORY_INPUT, "[GameEngine] Publishing keyboard event - Key: %s, Pressed: false", keyNameLower.c_str());
                events::EventManager::getInstance().publish(keyboardEvent);
            }
        }
