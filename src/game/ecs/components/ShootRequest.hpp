...
public:
   ...
   */
  ShootRequest(Entity entity, float x, float y, float dirX, float dirY);
   ...
   */
  void
  markProcessed(std::optional<Entity::ID> projectileEntityId = std::nullopt);
  ...
  Vector2 getPositionCopy() const {
    return position;
  } // Returns a copy for safety
  const Vector2 &getDirection() const { return direction; }
  Vector2 getDirectionCopy() const { return direction; }
  std::chrono::steady_clock::time_point getTimestamp() const {
    return timestamp;
  }
  bool isProcessed() const { return processed; }
  std::optional<Entity::ID> getProjectileEntityId() const {
