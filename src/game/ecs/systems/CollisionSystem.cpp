...
CollisionSystem::CollisionSystem()
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                "[CollisionSystem] Initialized with pure ECS architecture (component-only)");
}
    ...
    for (size_t i = 0; i < entities.size(); i++) {
        for (size_t j = i + 1; j < entities.size(); j++) {
            ...
            if (checkCollision(entityA, entityB)) {
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                           "[CollisionSystem] Collision detected between entity %llu and entity %llu", 
...
void CollisionSystem::clearCollisionResults() {
    // Clear collision results for all entities that have CollisionResult components
    const auto& allEntities = getEntities();
    int clearedEntities = 0;
    
    for (const Entity& entity : allEntities) {
        auto* collisionResult = componentManager_.getComponent<components::CollisionResult>(entity);
    ...
    SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION,
                "[CollisionSystem] Cleared collision results for %d entities", clearedEntities);
}

void CollisionSystem::storeCollisionResult(const Entity& entityA, const Entity& entityB) {
    ...
    auto* collisionResultA = componentManager_.getComponent<components::CollisionResult>(entityA);
    if (collisionResultA != nullptr && collisionResultA->isEnabled()) {
        collisionResultA->addCollision(entityA, entityB, info.collisionPoint, info.collisionNormal);
        SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION,
                    "[CollisionSystem] Stored collision result for entity %llu", entityA.getId());
    }
    
    // Store collision in entity B
    auto* collisionResultB = componentManager_.getComponent<components::CollisionResult>(entityB);
    if (collisionResultB != nullptr && collisionResultB->isEnabled()) {
        collisionResultB->addCollision(entityA, entityB, info.collisionPoint, info.collisionNormal);
        SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION,
                    "[CollisionSystem] Stored collision result for entity %llu", entityB.getId());
    }
}
...
void CollisionSystem::ensureCollisionResultComponent(const Entity& entity) {
    ...
    if (collisionResult == nullptr) {
        ...
        systemManager_.onComponentAdded(entity, std::type_index(typeid(components::CollisionResult)));
        SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION,
                    "[CollisionSystem] Created CollisionResult component for entity %llu", entity.getId());
    }
}

bool CollisionSystem::checkCollision(const Entity& entityA, const Entity& entityB) {
    auto* transformA = componentManager_.getComponent<components::Transform>(entityA);
    auto* spriteA = componentManager_.getComponent<components::Sprite>(entityA);
    auto* transformB = componentManager_.getComponent<components::Transform>(entityB);
    ...
    return (
        transformA->getPosition().x < transformB->getPosition().x + spriteB->getWidth() &&
        transformA->getPosition().x + spriteA->getWidth() > transformB->getPosition().x &&
        transformA->getPosition().y < transformB->getPosition().y + spriteB->getHeight() &&
        transformA->getPosition().y + spriteA->getHeight() > transformB->getPosition().y
    );
}

CollisionSystem::CollisionInfo CollisionSystem::calculateCollisionInfo(const Entity& entityA, const Entity& entityB) {
    auto* transformA = componentManager_.getComponent<components::Transform>(entityA);
    auto* spriteA = componentManager_.getComponent<components::Sprite>(entityA);
    auto* transformB = componentManager_.getComponent<components::Transform>(entityB);
    auto* spriteB = componentManager_.getComponent<components::Sprite>(entityB);
    ...
    Vector2 centerA(
        transformA->getPosition().x + spriteA->getWidth() / 2.0f,
        transformA->getPosition().y + spriteA->getHeight() / 2.0f
    );
    Vector2 centerB(
        transformB->getPosition().x + spriteB->getWidth() / 2.0f,
...
std::string CollisionSystem::toString() const {
    return "CollisionSystem(entities=" + std::to_string(getEntities().size()) + ", pure-ECS)";
}
