...
namespace game::ecs::systems {
    ...
    PlayerControlSystem::PlayerControlSystem(game::events::EventManager& eventManager, 
                                           float worldWidth, 
                                           float worldHeight)
        : System()
        ...
        , worldHeight_(worldHeight)
    {
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                   "PlayerControlSystem initialized with dual input support, world size: %.1fx%.1f", 
                   worldWidth_, worldHeight_);
    }
    
    void PlayerControlSystem::onEvent(const game::events::Event& event) {
        // Check if this is a keyboard event
        const auto* keyboardEvent = dynamic_cast<const game::events::KeyboardEvent*>(&event);
        if (!keyboardEvent) {
    ...
    void PlayerControlSystem::update(float deltaTime) {
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                   "PlayerControlSystem update: game_state=%s, entities=%zu, pressed_keys_count=%zu",
                   gameState.getStateString().c_str(), getEntities().size(), pressedKeys_.size());
        ...
        for (const auto& entity : getEntities()) {
            ...
            auto* transform = getComponentManager()->getComponent<game::ecs::components::Transform>(entity);
            auto* player = getComponentManager()->getComponent<game::ecs::components::Player>(entity);
            auto* input = getComponentManager()->getComponent<game::ecs::components::Input>(entity);
            
            // Get optional KeyboardInput component for dual input support
            auto* keyboardInput = getOptionalComponent<game::ecs::components::KeyboardInput>(entity);
            ...
            if (keyboardInput && keyboardInput->isEnabled()) {
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                           "PlayerControlSystem: Entity %llu using KeyboardInput component (primary)", 
                           entity.getId());
            } else {
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                           "PlayerControlSystem: Entity %llu using event-driven input (fallback)", 
                           entity.getId());
            }
    ...
    void PlayerControlSystem::handleMovement(const Entity& entity, 
        ...
        if (dx != 0.0f) {
            ...
            auto* sprite = getComponentManager()->getComponent<game::ecs::components::Sprite>(entity);
            if (sprite) {
            ...
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                       "PlayerControlSystem: Moving from %.1f to %.1f (dx=%.2f)", 
                       transform->getPosition().x, newX, dx);
            
            transform->setPosition(newX, transform->getPosition().y);
        }
    }
    
    void PlayerControlSystem::handleShooting(const Entity& entity,
                                           game::ecs::components::Transform* transform,
        ...
        // Get fire key - check for both "fire" and "space" mappings (matches Java logic)
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                   "PlayerControlSystem: Fire key mapped to: '%s'", 
                   fireKey.c_str());
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                   "PlayerControlSystem: Fire key '%s' pressed: %s", 
                   fireKey.c_str(), fireKeyPressed ? "YES" : "NO");
        ...
        if (fireKeyPressed) {
            ...
            if (player->canFire()) {
                ...
                // NEW: Create ShootRequest component instead of directly creating projectile
    ...
    bool PlayerControlSystem::isKeyPressed(const std::string& key, 
    ...
    void PlayerControlSystem::createShootRequest(const Entity& entity, float x, float y) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                   "PlayerControlSystem.createShootRequest: Creating shoot request for entity %llu at (%.1f, %.1f)", 
                   entity.getId(), x, y);
        
        try {
            // Calculate missile starting position (matches existing createProjectile logic)
            ...
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                       "PlayerControlSystem: Added ShootRequest component at (%.1f, %.1f)", 
                       x, missileStartY);
            
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                       "🎯 PlayerControlSystem: SHOOT REQUEST CREATED - Entity %llu at (%.1f, %.1f)", 
    ...
    void PlayerControlSystem::createProjectile(float x, float y) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                   "PlayerControlSystem.createProjectile: Starting projectile creation at (%.1f, %.1f)", 
                   x, y);
        ...
        try {
            ...
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                       "PlayerControlSystem: Added Transform at (%.1f, %.1f)", 
                       x, missileStartY);
            ...
            float maxRange = missileStartY - 10.0f; // Distance from missile start to screen top minus buffer
            getComponentManager()->addComponent<game::ecs::components::Projectile>(
                projectileEntity, 400.0f, maxRange);
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                       "PlayerControlSystem: Added Projectile component (speed=400, range=%.1f)", 
                       maxRange);
            ...
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, 
                       "🎯 PlayerControlSystem: PROJECTILE CREATED - Entity %llu with range %.1f", 
