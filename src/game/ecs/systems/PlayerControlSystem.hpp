...
/**
 * PlayerControlSystem enhanced for dual input support and pure component-based
 * input.
 *
 * C++ Implementation References:
 * - Python:
 * /Lesson-41-CleanEvents/Python/src/game/ecs/systems/player_control_system.py
 * - Java:
 * /Lesson-41-CleanEvents/Java/src/game/ecs/systems/PlayerControlSystem.java
 *
 * System that handles player input for movement and shooting with dual input
 * support:
 ...
 */
class PlayerControlSystem : public game::ecs::System,
                            public game::events::EventListener {
public:
  /**
   * Constructor
   *
   ...
   * Uses dual input approach: KeyboardInput components first, then fallback to
   * events
   *
...
private:
   ...
   */
  void createShootRequest(const Entity &entity, float x, float y, float dirX,
                          float dirY);

  /**
   * Create a new projectile at the specified position (preserved for
   * compatibility)
   *
   * @param x X position to create the projectile
   * @param y Y position to create the projectile
