...
void Image::load(const std::string &path, SDL_Renderer *renderer) {
  ...
  if (!renderer) {
    std::cerr << "Failed to load image: " << path << " - Invalid renderer"
              << std::endl;
...
void Image::render(SDL_Renderer *renderer, int x, int y, int width, int height,
  ...
  SDL_RenderTextureRotated(renderer, texture, nullptr, &destRect, rotation,
                           &center, SDL_FLIP_NONE);
}

void Image::renderScaled(SDL_Renderer *renderer, int x, int y, int width,
                         int height, ScalingMode mode) {
  if (!texture || !renderer)
    return;
  ...
  case ScalingMode::TILE: {
    ...
    for (int ty = 0; ty < tilesY; ++ty) {
      for (int tx = 0; tx < tilesX; ++tx) {
        ...
        int tileW = std::min(static_cast<int>(texWidth),
                             width - static_cast<int>(tx * texWidth));
        int tileH = std::min(static_cast<int>(texHeight),
                             height - static_cast<int>(ty * texHeight));
  ...
  case ScalingMode::CENTER: {
    ...
    render(renderer, centerX, centerY, static_cast<int>(texWidth),
